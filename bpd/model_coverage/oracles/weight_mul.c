...
int main(int c,char**v){
  ggml_cpu_init(); int ne0=2048,ne1=2,n=ne0*ne1;
  float* a=ldf(v[1],n); float* w=ldf(v[2],ne0); float* xr=ldf(v[3],n);
  struct ggml_init_params p={64*1024*1024,NULL,false}; struct ggml_context* ctx=ggml_init(p);
  struct ggml_tensor* A=ggml_new_tensor_2d(ctx,GGML_TYPE_F32,ne0,ne1); memcpy(A->data,a,n*4);
  struct ggml_tensor* W=ggml_new_tensor_1d(ctx,GGML_TYPE_F32,ne0); memcpy(W->data,w,ne0*4);
  struct ggml_tensor* r=ggml_mul(ctx,A,W);  // broadcast W [2048] across rows
  ...
  printf("attn_norm MUL: maxULP=%d nz=%d/%d out[0]=%.9g ref[0]=%.9g %s\n",mx,nz,n,o[0],xr[0],mx==0?"GREEN":"DIV");return 0;
}
